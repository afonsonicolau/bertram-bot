...
async def get_character(steamid, message):
    if sql.open_connection():
        ...
        cursor.execute("SELECT identifier, firstname, lastname FROM users WHERE identifier LIKE %(steamid)s", {'steamid': '%' + steamid})
        ...
        if len(characters) > 0:
            ...
            for character in characters:
                formatted_characters += "\nIdentificador - " + character[0] + " | Nome - " + character[1] + " " + character[2]
